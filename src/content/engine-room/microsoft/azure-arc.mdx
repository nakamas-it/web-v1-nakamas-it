...
Azure Arc extends the Azure control plane outward to any server, regardless of where it runs, by projecting it as an Azure resource through a lightweight outbound agent. Once enrolled, Arc-enabled servers inherit Azure Policy enforcement, RBAC, Update Manager, VM extensions, and Defender for Cloud coverage — creating a single management plane for hybrid and multi-cloud server estates. Understanding Arc as an extension of Azure governance rather than a migration tool is the key conceptual distinction.
